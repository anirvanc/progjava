...
public class DepartmentController {
    ...
    public String index (final Model model, @RequestParam(value = "q", required = false) String query) {
        ...
        if (StringUtils.hasLength(query)) {
            departmentPage = this.departmentRepository.findByNameLike(query, new PageRequest(0, 100));
            model.addAttribute("q", query);
        } else {
            departmentPage = this.departmentRepository.findAll(new PageRequest(0, 100));
        }
